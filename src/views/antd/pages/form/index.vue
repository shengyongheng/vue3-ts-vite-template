...
<template>
  <el-form ref="formRef" :model="form.formData" label-width="auto" style="max-width: 600px">
    <el-form-item v-for="item in form.formItems as unknown as any" :key="item.prop" :prop="item.prop"
      ...
      <el-select v-if="item.type === 'Select'" v-model="form.formData[item.prop]" :placeholder="item.placeholder"
        :multiple="item!.multiple">
        <el-option v-for="option in item.options" :key="option.value" :label="option.label" :value="option.value" />
      </el-select>
    </el-form-item>
    <el-form-item>
      <slot :form-ref="formRef"></slot>
    </el-form-item>
  </el-form>
</template>
